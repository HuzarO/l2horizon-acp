...
from ..models import (
    FishingRod, Fish, FishingHistory, FishingBait,
...
def fishing_game_cast(request):
    ...
    # Custo de cada tentativa (1 ficha)
    FISHING_COST = 1
    
    user = request.user
    if user.fichas < FISHING_COST:
        return JsonResponse({
            'error': _('Você não tem fichas suficientes. Necessário: 1 ficha')
        }, status=400)
    
    # Deduzir ficha
    user.fichas -= FISHING_COST
